---
layout:     post                        # 使用的布局(必填)
title:      帝华                     # 标题(必填)
subtitle:   工作日志不提供评论                         # 副标题(必填)
date:       2019-08-23                  # 时间(必填)
author:     Mambo723                    # 作者(必填)
gitalk_enable: false                     # 是否开启评论(必填)
---
<!-- ![logo](http://www.dh.cn/imgs/DH.png) -->

> 奇文共欣赏，疑义相与析。


---

### 指导规范
* [新人入职](newplayer)
* [客户端人员分工参考](深圳客户端人员分工参考)
* [工程简介](工程简介)
* [热更新编码规范](热更新编码规范)
* [热更包生成及发布流程-V2.0](热更包生成及发布流程-V2.0)
* [定位 Bugly 报错](定位Bugly报错)
* [子游戏开发及交付对接规范](子游戏开发及交付对接规范)
* [捕鱼项目结构、自动化工具及相关规范](捕鱼项目结构、自动化工具及相关规范)

---

### 日志
* [捕鱼优化及典型Bug修复日志](捕鱼优化及典型Bug修复日志)
* [关于“使用 CocosCreator 制作3D游戏”的可行性报告](关于CocosCreator开发3D游戏的可行性报告)
* [二期项目交接文档](二期项目交接文档)

---

### 交付对接文档

---

### 功能说明
* [psd2node插件使用方法](psd2node)
* [子游戏移植工具使用方法](子游戏移植工具)
* [服务端与客户端通信加解密流程](服务端与客户端通信加解密流程)

---

### 技术分享


* [Markdown 教程](https://www.cnblogs.com/liugang-vip/p/6337580.html)
* [source-map 技术支持资料](https://www.jianshu.com/p/fc622f8cff99)
* [SVN 常用命令详解](https://blog.csdn.net/GX_1_11_real/article/details/86489756)
* [Git 常用命令详解](https://blog.csdn.net/weelyy/article/details/82823798)
* [Live Share](https://devblogs.microsoft.com/visualstudio/visual-studio-live-share-for-real-time-code-reviews-and-interactive-education/)
* [Windows平台如何修改SVN文件名的大小写](https://blog.csdn.net/foupwang/article/details/81735947)

---

### 相关网站
* [帝华官网](http://www.dh.cn)
* [Cocos官网](https://www.cocos.com)
* [开源中国](https://www.oschina.net)
* [游资网](https://www.gameres.com)
* [996.icu](https://996.icu)
